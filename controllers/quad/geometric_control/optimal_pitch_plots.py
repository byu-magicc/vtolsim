...
def plot_constant_F_V_changing_gamma(save=False, nsamples=200):
    ...
    F_d_d = np.array([0., 0., -QUAD.mass*QUAD.gravity])
    Va = 3.
...
def plot_constant_V_gamma_changing_F(save=False, nsamples=200):
    ...
    F_const = np.array([0., 0., -QUAD.mass*QUAD.gravity])
    F_x = np.arange(-QUAD.mass*QUAD.gravity, QUAD.mass*QUAD.gravity, .01)
    Va = 3.
    gamma = 0.
...
def plot_constant_F_gamma_increasing_V(save=False, nsamples=200):
    ...
    F_d_d = np.array([2., 0., -QUAD.mass*QUAD.gravity])
    Va = np.arange(.1, 50, .1)
    gamma = 0.
