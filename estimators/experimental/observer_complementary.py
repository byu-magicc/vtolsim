...
class Observer:
    def __init__(self, ts_control, initial_state = MsgState(), initial_measurements = MsgSensors()):
        ...
        self.attitude_complementary = ComplementaryAttitude(SIM.ts_simulation)
    ...
    def update(self, measurement):
        ...
        # estimate phi and theta with complementary filter
        self.attitude_complementary.update(measurement, self.estimated_state)
...
class ComplementaryAttitude:
    def __init__(self, Ts=0.01):
        ...
        self.Ts = Ts
        self.roll_hat = 0
        self.roll_bias_hat = 0
        self.pitch_hat = 0
        self.pitch_bias_hat = 0
        self.lpf_accel_x = AlphaFilter(alpha=0.0, y0=0)
        self.lpf_accel_y = AlphaFilter(alpha=0.0, y0=0)
        self.lpf_accel_z = AlphaFilter(alpha=0.0, y0=CTRL.gravity)
    ...
    def update(self, measurement, state):
        ...
        phi_accel = np.arctan2(-accel_y, -accel_z)
        theta_accel = np.arcsin(accel_x / CTRL.gravity)

        roll_error = phi_accel - self.roll_hat
        self.roll_bias_hat = self.roll_bias_hat + self.Ts * (-self.ki * roll_error)
        self.roll_hat = self.roll_hat + self.Ts * \
                         ( (measurement.gyro_x - self.roll_bias_hat)
                             + self.kp * roll_error )
        # self.roll_hat = self.roll_hat + self.Ts * \
        #                  ( measurement.gyro_x + self.kp * roll_error )
        pitch_error = theta_accel - self.pitch_hat
        self.pitch_bias_hat = self.pitch_bias_hat + self.Ts * (self.ki * pitch_error)
        self.pitch_hat = self.pitch_hat + self.Ts * \
                         ( (measurement.gyro_y - self.pitch_bias_hat)
                             + self.kp * pitch_error )
        # self.pitch_hat = self.pitch_hat + self.Ts * \
        #                  ( measurement.gyro_y + self.kp * pitch_error )
        state.phi = self.roll_hat
        state.theta = self.pitch_hat
        state.bx = self.roll_bias_hat
        state.by = self.pitch_bias_hat
