...
class Observer:
    ...
    def update(self, measurement):
        ...
        self.attitude.update(measurement, self.estimated_state)
...
class Complementary:
    # implement complementary filter to estimate roll and pitch angles plus gyro biases
    def __init__(self):
        self.N = 1  # number of prediction step per sample
        self.bxhat = 0  # bias on p
        self.phihat = 0  # estimate of roll angle
        self.byhat = 0  # bias on q
        self.thetahat = 0  # estimate of pitch angle
        self.Ts = SIM.ts_control/self.N
        self.ki = 0.02
        self.kp = 0.01

    def update(self, measurement, state):
        phi_accel = np.arctan(measurement.accel_y / measurement.accel_z)
        theta_accel = np.arcsin(measurement.accel_x/CTRL.gravity)
        for i in range(0, self.N):
            e_phi = phi_accel - self.phihat
            self.bxhat = self.bxhat + self.Ts * (-self.ki * e_phi)
            self.phihat = self.phihat + self.Ts * (
                (measurement.gyro_x - self.bxhat) + self.kp * e_phi)
            e_theta = theta_accel - self.thetahat
            self.byhat = self.byhat + self.Ts * (-self.ki * e_theta)
            self.thetahat = self.thetahat + self.Ts * (
                (measurement.gyro_y - self.byhat) + self.kp * e_theta)
        state.phi = self.phihat
        state.theta = self.thetahat
        state.bx = self.bxhat
        state.by = self.byhat
